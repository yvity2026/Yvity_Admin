      ...
      <div className="flex-1 flex flex-col justify-between">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
          <div>
            ...
            <p className="text-xs text-gray-500 font-medium mt-0.5">{data.subtitle}</p>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-6">
          {data.tags.map((tag, i) => (
            <div key={i} className="flex items-center gap-1.5 bg-[#F0F7F6] text-[#124B48] px-3 py-1.5 rounded-full text-[11px] font-bold tracking-wide">
              <span>{tag.icon}</span>
              {tag.label}
            </div>
          ))}
