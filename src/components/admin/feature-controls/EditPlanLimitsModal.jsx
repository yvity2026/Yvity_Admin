...
function FieldRow({ field, value, onChange }) {
  // Local raw string so the user can freely type/clear without being snapped
  // back to "Unlimited" on every keystroke. Normalize only on blur.
  const displayValue = value === "Unlimited" ? "Unlimited" : String(value ?? "");
  const [raw, setRaw] = useState(displayValue);

  useEffect(() => {
    setRaw(value === "Unlimited" ? "Unlimited" : String(value ?? ""));
  }, [value]);
