...
const isDefaultActions =
  ...
  return (
    ...
    `}
      >
        <div className="sticky top-0 z-10 bg-[#0A4A4A]">
          {/* Website Logo */}
          ...
          <motion.div
            ...
            className="mb-2 flex flex-col pl-10 pt-8"
          >
            <div className="w-[142px] h-full flex flex-col">
              <div className={`${collapsed ? "w-10 h-10" : "w-14 h-14"} rounded-full bg-white`}></div>
              {
                collapsed ? (<p className="text-[#F8F6F1] font-poppins text-base font-semibold">
                {initials}
              </p>) : (<p className="text-[#F8F6F1] font-poppins text-base font-semibold">
                Krishna Mohan
              </p>)
              }
              
              <button className="flex gap-2 px-4 md:py-[6px] rounded-2xl bg-[rgba(245,158,11,0.2)]">
                <FaCrown />
                {!collapsed && <span className="text-xs">Gold Member</span>}
              </button>
            </div>
          </motion.div>
                ...
                {section.navitems.map((item, j) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`flex items-center gap-4 font-semibold px-10 py-2 cursor-pointer rounded-lg transition-colors duration-200
      ...
      <div className="flex flex-col flex-1 ">
        ...
        <header className="h-[60px] sticky top-0 z-10 bg-white flex items-center justify-between border-b px-[90px]">
          ...
          <div className="hidden h-10 md:flex gap-4 py-[10px] items-center">
            ...
            {currentHeader.actions.includes("notifications") && (
              <motion.button className=" relative rounded-full h-10 w-10 bg-green-300 flex items-center justify-center"><FaBell className="h-5 w-5" /><span className="absolute top-2 right-3 w-1 h-1 bg-[#065F46] rounded-full animate-pulse"></span></motion.button>
            )}

            {currentHeader.actions.includes("profile") && (
              <motion.button className="rounded-full h-10 w-10 bg-yellow-500" ><p className="font-bold text-sm">KM</p></motion.button>
            )}
