...
function Sidebar({ activeNav, setActiveNav, onClose }) {
  return (
    <div className="bg-[#0A4A4A] min-h-screen w-[280px] shrink-0 flex flex-col">
      ...
      {/* User */}
      <div className="px-4 py-[14px] border-b border-[#155e5e]">
      ...
      <div className="flex-1">
        {Object.entries(navItems).map(([section, items]) => (
          <div key={section}>
            <div className="text-[#5fa8a8] text-[10px] font-semibold uppercase tracking-widest px-4 pt-[14px] pb-1">
              {section}
            </div>
            {items.map((item) => {
              const isActive = activeNav === item.label;
              return (
                ...
                >
                  <div
                    className={`flex items-center gap-[10px] px-4 py-[9px] cursor-pointer text-[13px] transition-colors
                      ${isActive
                        ? "text-white bg-[#155e5e] border-l-[3px] border-[#8bc34a]"
                        : "text-[#a3d0d0] border-l-[3px] border-transparent hover:bg-[#155e5e]"
                      }`}
                  >
                    {item.icon}
                    {item.label}
      ...
      {/* Logout */}
      <div className="py-4">
        <div className="flex items-center gap-[10px] px-4 py-[9px] text-red-500 text-[13px] cursor-pointer hover:bg-[#155e5e] transition-colors">
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width={16} height={16}>
            <path d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h6a2 2 0 012 2v1" strokeWidth="2" />
          </svg>
          Logout
...
export default function SettingsPage() {
  ...
  return (
    <div className="flex min-h-screen font-sans bg-gray-100">
      ...
      {/* Sidebar */}
      <div
        className={`fixed top-0 left-0 h-screen z-50 transition-transform duration-300 md:relative md:translate-x-0
